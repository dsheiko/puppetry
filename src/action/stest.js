...
const actions = createActions({
  ...
  /**
    ...
    * @returns {object}
    */
  INSERT_ADJACENT_STEST: ( options, position, id = null ) => ({
    position,
    options,
    id
  }),
  /**
    ...
    * @returns {object}
    */
  UPDATE_STEST: ( options ) => validate( options, { ...I.ENTITY, ...I.TEST, ...I.UPDATE }),
  /**
...
actions.swapTest = ( payload ) => async ( dispatch, getState ) => {
  ...
  try {
    const { sourceInx, targetInx, sourceId, targetId } = payload,
          tests = getStestsFlat( getState().suite.groups ),
