...
export class EditableCell extends React.Component {
	...
	render() {
		const {updateRecord, placeholder, dataIndex, record, prefixIcon, className} = this.props,
			{editing} = record,
			error = this.state.error,
			value = this.props.record[dataIndex];
		console.warn("-- Console WHAT IS", this.props);
		return (
			<div className="editable-cell" onDoubleClick={() => {
				console.warn(record);
				// record.editing = true;
